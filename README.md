...
GitHub Actions checks at 04:00 UTC on Monday through Friday. Scheduled or manual scans open or update a pull request containing only `ref.bib` and `aux.yml`; the normal build-and-deploy job runs when that pull request is merged into `main`. Known arXiv IDs—those already represented in `ref.bib` or in an arXiv/e-print link in the matching `aux.yml` record—are removed before update processing. Abstract retrieval is limited to matching BibTeX records without an abstract, plus recent Atom entries that will be added to `ref.bib`; aux-only records without a BibTeX match are ignored. The updater makes one Atom-feed request per configured author, with requests spaced to reduce rate limiting. 429 responses honor arXiv's `Retry-After` value with exponential backoff. The request delay, retry count, and maximum retry wait can be adjusted with `ARXIV_REQUEST_DELAY_MS`, `ARXIV_RETRY_ATTEMPTS`, and `ARXIV_MAX_RETRY_DELAY_MS`.
