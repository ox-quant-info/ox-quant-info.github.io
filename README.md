...
The updater reads each configured public arXiv author `.atom` feed, which provides separate author elements, filters entries by the Atom `updated` date and configured lookback window, and uses those entries directly for BibTeX metadata and verbatim abstracts. An entry whose Atom `updated` and `published` timestamps are equal is treated as a new submission: it is added as a new `@misc` record only when absent, and an abstract is added only when missing. Existing fields are never overwritten for new submissions. An entry whose timestamps differ is treated as a replacement: title, authors, and abstract are updated only when the matching arXiv record already exists as `@misc` in `ref.bib`; updated papers without a matching `@misc` record are ignored. Existing `@article`, `@inproceedings`, thesis, and other non-`misc` records are not modified. Out-of-range entries are discarded before any BibTeX or abstract update; no export API metadata query is made. New BibTeX authors use `Last, First` form, and new citation keys use `firstauthorlastnameYearFirstImportantWord`, with a numeric suffix for collisions. It is safe to run repeatedly: arXiv ID, DOI, normalized title, and auxiliary-feed checks prevent duplicates.

GitHub Actions runs Monday through Friday at the UTC time configured in `.github/workflows/build.yml`. The direct updater scans member author feeds and global replacement entries before one combined commit to `main`; the site is then built and deployed in the same workflow. The pull-request updater scans the daily `https://rss.arxiv.org/atom/quant-ph` feed for global additions without a lookback window. Global replacements are selected by arXiv ID already represented in the local data, while global `new` and `cross` entries are matched by canonical member names and `alt_name` values from the PI and current-member YAML records. Global-feed `replace` and `replace-cross` entries are applied directly to eligible local `@misc` records, while global-feed `new` and `cross` entries missing from the local bibliography or auxiliary data are staged for a fallback pull request. A cross-list entry is not given a special label. Known arXiv IDs already represented in `ref.bib` or `aux.yml` are excluded from new additions. Network and transient HTTP errors are retried, while permanent HTTP errors stop immediately. A 429 response honors arXiv's `Retry-After` value with capped exponential backoff; all transient errors share a maximum of five total retries per request. The request delay, retry budget, and maximum retry wait can be adjusted with `ARXIV_REQUEST_DELAY_MS`, `ARXIV_MAX_RETRIES`, and `ARXIV_MAX_RETRY_DELAY_MS`. If the repository setting preventing `GITHUB_TOKEN` from creating pull requests cannot be changed, configure a fine-grained `ARXIV_PR_TOKEN` repository secret with Contents and Pull requests write access.
