...
The updater reads each configured public arXiv author `.atom` feed, which provides separate author elements, filters entries by the Atom `updated` date and configured lookback window, and uses those entries directly for BibTeX metadata and verbatim abstracts. A recent first submission is added as a new `@misc` record. A recent version update edits title, author, and `abs` fields only when the matching arXiv record already exists as `@misc` in `ref.bib` and its arXiv ID is not listed in `aux.yml`; updated papers without a matching `@misc` record are ignored. Existing `@article`, `@inproceedings`, thesis, and other non-`misc` records are not modified. ArXiv IDs listed in `aux.yml` are protected from metadata and abstract updates. Out-of-range entries are discarded before any BibTeX or abstract update; no export API metadata query is made. New BibTeX authors use `Last, First` form, and new citation keys use `firstauthorlastnameYearFirstImportantWord`, with a numeric suffix for collisions. It is safe to run repeatedly: arXiv ID, DOI, and normalized title checks prevent duplicates.

GitHub Actions runs at 04:00 UTC on Monday through Friday. The updater runs whenever the scheduled workflow starts, without rejecting delayed GitHub Actions starts because they fall outside the nominal hour. Scheduled or manual scans open or update a pull request containing only `ref.bib` and `aux.yml`; the normal build-and-deploy job runs when that pull request is merged into `main`. Known arXiv IDs—those already represented in `ref.bib` or in an arXiv/e-print link in the matching `aux.yml` record—are removed before update processing. Abstract retrieval is limited to matching BibTeX records without an abstract, plus recent Atom entries that will be added to `ref.bib`; aux-only records without a BibTeX match are ignored. The updater makes one Atom-feed request per configured author, with requests spaced to reduce rate limiting. Network and transient HTTP errors are retried, while permanent HTTP errors stop immediately. A 429 response honors arXiv's `Retry-After` value with capped exponential backoff; all transient errors share a maximum of five total retries per request. The request delay, retry budget, and maximum retry wait can be adjusted with `ARXIV_REQUEST_DELAY_MS`, `ARXIV_MAX_RETRIES`, and `ARXIV_MAX_RETRY_DELAY_MS`.
